...
| 033 | client-dynamic-routes ✅                 | 客户端     | 030, 031, 032 | 消费端动态路由(功能链收官):routesync 聚合双视图两端共用;Windows tunnel.SetExtraRoutes(IpcSet replace_allowed_ips 原地热更新不断流+netsh/netlink 路由差分)+panel 60s 对账+「可达子网」区块;OpenWrt engine.SetRoutes(ReplaceAllowedIPs+路由差分)+032 对账循环双路消费(排除自己防回环)+announce list --all;daemon OnUp 即时对账;服务端零改动;两端真流量 CI 验收 |
